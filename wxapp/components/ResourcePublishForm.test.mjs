...
test('resource publish form supports address fields with optional map location', () => {
  ...
  assert.match(source, /uni\.chooseLocation\(\{[\s\S]*latitude[\s\S]*longitude[\s\S]*未获取到详细地址，请搜索并选择具体地点[\s\S]*地图位置已保存[\s\S]*resolveChooseLocationErrorMessage\(err\)/)
  assert.match(source, /function resolveChooseLocationAddress\(result, key\) \{[\s\S]*buildChooseLocationAddressText\(result\)[\s\S]*normalizeChooseLocationText\(getDynamicAddressText\(key\)\)[\s\S]*return ''/)
