...
test('my page shows merchant logo and name when merchant profile exists', () => {
  ...
  assert.match(source, /await Promise\.all\(\[loadMerchantProfile\(\), loadMerchantMetricsSummary\(\)\]\)/)
...
test('my page exposes vip entry and hides merchant verification main path', () => {
  const source = fs.readFileSync(path.join(root, 'pages/my/index.vue'), 'utf8')

  assert.match(source, /<text class="action-title">VIP 权益<\/text>/)
  assert.match(source, /<text class="action-meta">80 条发布额度/)
