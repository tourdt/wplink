...
test('market page expands secondary categories inline from a compact arrow control', () => {
  ...
  assert.match(source, /<scroll-view[\s\S]*class="type-panel"[\s\S]*<view class="type-panel-content">[\s\S]*<view class="type-panel-head">[\s\S]*<view class="type-panel-grid">/)
  ...
  assert.match(cssBlock('.type-panel'), /padding:\s*0;/)
  assert.match(cssBlock('.type-panel'), /overflow:\s*hidden;/)
  assert.match(cssBlock('.type-panel'), /border-radius:\s*12rpx;/)
  assert.match(cssBlock('.type-panel-content'), /padding:\s*16rpx;/)
