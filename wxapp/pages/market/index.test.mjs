...
test('supply demand page shows direction tabs and category controls', () => {
  ...
  assert.match(source, /const directionTabs = \[[\s\S]*\{ label: '供应', value: RESOURCE_DIRECTION_SUPPLY \}[\s\S]*\{ label: '需求', value: RESOURCE_DIRECTION_DEMAND \}[\s\S]*\]/)
  assert.doesNotMatch(source, /label: '找供需信息'/)
...
test('supply recommendation empty state uses concise copy without image text', () => {
  ...
  assert.match(source, /const recommendationEmptyTitle = computed\(\(\) => activeDirection\.value === RESOURCE_DIRECTION_DEMAND \? '暂无推荐需求' : '暂无推荐供应'\)/)
  assert.match(source, /<text class="empty-desc">换个类型或搜索关键词。<\/text>/)
  assert.doesNotMatch(source, /<text>供需信息<\/text>/)
  assert.doesNotMatch(source, /当前类型暂无推荐供需信息/)
