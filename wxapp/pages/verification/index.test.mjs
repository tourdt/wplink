...
test('verification page reuses merchant profile identity instead of asking users to choose', () => {
  ...
  assert.match(source, /const certifiableVerificationTypes = \['factory', 'stall', 'stockist', 'service_provider'\]/)
  assert.match(source, /const isUnsupportedVerificationType = computed\(\(\) => !certifiableVerificationTypes\.includes\(form\.verificationType\)\)/)
  assert.match(source, /当前身份暂不支持认证/)
...
test('verification pending state shows review progress instead of editable form', () => {
  ...
  assert.match(source, /const showVerificationForm = computed\(\(\) => !isVerificationPending\.value && !isVerificationPaymentPending\.value && !isUnsupportedVerificationType\.value && \(!isVerificationVerified\.value \|\| changingVerifiedCertification\.value\)/)
...
test('verification page validates required certification evidence before submitting', () => {
  ...
  assert.match(source, /if \(isUnsupportedVerificationType\.value\) return '当前身份暂不支持认证，请先调整主要身份'/)
