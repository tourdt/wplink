...
func (l *AdminPermissionLogic) ListOperators(ctx context.Context, req ListAdminOperatorsReq, actor AdminPermissionActor) (ListAdminOperatorsResp, error) {
	...
	if err != nil {
		LogAdminFailure(ctx, "查询后台管理员账号失败", "list_admin_operators", err,
			logx.Field("operatorId", actor.OperatorID),
			logx.Field("roleFiltered", role != ""),
			logx.Field("statusFiltered", status != ""),
		)
...
func (l *AdminPermissionLogic) CreateOperator(ctx context.Context, req SaveAdminOperatorReq, actor AdminPermissionActor) (SaveAdminOperatorResp, error) {
	...
	input, err := l.buildOperatorInput(ctx, "", req, actor, true)
	if err != nil {
		return SaveAdminOperatorResp{}, err
	}
...
func (l *AdminPermissionLogic) UpdateOperator(ctx context.Context, operatorID string, req SaveAdminOperatorReq, actor AdminPermissionActor) (SaveAdminOperatorResp, error) {
	...
	input, err := l.buildOperatorInput(ctx, operatorID, req, actor, false)
	if err != nil {
		return SaveAdminOperatorResp{}, err
	}
...
func (l *AdminPermissionLogic) ListModulePermissions(ctx context.Context, actor AdminPermissionActor) (AdminModulePermissionsResp, error) {
	...
	if err != nil {
		LogAdminFailure(ctx, "查询后台角色模块权限失败", "list_admin_module_permissions", err,
			logx.Field("operatorId", actor.OperatorID), logx.Field("roleCode", permission.RolePlatformOperator))
...
func (l *AdminPermissionLogic) UpdateRoleModulePermissions(ctx context.Context, roleCode string, req UpdateAdminRoleModulePermissionsReq, actor AdminPermissionActor) (SaveAdminRoleModulePermissionsResp, error) {
	...
	if err != nil {
		...
		LogAdminFailure(ctx, "保存后台角色模块权限失败", "update_admin_role_modules", err,
			logx.Field("operatorId", actor.OperatorID), logx.Field("roleCode", roleCode), logx.Field("moduleCount", len(modules)))
...
func (l *AdminPermissionLogic) buildOperatorInput(ctx context.Context, operatorID string, req SaveAdminOperatorReq, actor AdminPermissionActor, passwordRequired bool) (model.AdminOperatorInput, error) {
	...
	if passwordRequired || password != "" {
		...
		if err != nil {
			LogAdminFailure(ctx, "生成后台管理员密码哈希失败", "hash_admin_operator_password", err,
				logx.Field("operatorId", actor.OperatorID))
...
func mapAdminOperatorsResult(result model.ListAdminOperatorsResult) ListAdminOperatorsResp {
	...
	for _, item := range result.Items {
		items = append(items, AdminOperatorItem{
			...
			Status:      item.Status,
			Roles:       nonNilAdminStringSlice(item.Roles),
