...
func (l *ResourceReportLogic) ListResourceReports(ctx context.Context, req ListResourceReportsReq) (ListResourceReportsResp, error) {
	...
	if err != nil {
		logx.Errorf("加载资源举报列表失败: status=%s page=%d pageSize=%d err=%+v", status, req.Page, req.PageSize, err)
...
func (l *ResourceReportLogic) ReviewResourceReport(ctx context.Context, reportID string, req ReviewResourceReportReq) (ReviewResourceReportResp, error) {
	...
	if err != nil {
		logx.Errorf("审核资源举报失败: reportId=%s reviewerId=%s action=%s resourceAction=%s refundPublishQuota=%t err=%+v", input.ReportID, input.ReviewerID, input.Action, input.ResourceAction, input.RefundPublishQuota, err)
