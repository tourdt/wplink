# 服装产业带供需信息撮合平台 API 契约设计

> **文档状态：历史设计，已废弃**
>
> 本文仅保留早期方案背景，不作为当前实现、验收或生成依据。当前唯一 API 契约入口是 `backend/app/api/app.api`，生成一致性以仓库根目录 `make check-api-generated` 为准；本文正文中的端点、字段和流程不得用于判断线上能力。
...
- 支持小程序端、管理后台和后续城市站扩展。
- 保持 `resources` 统一供需信息模型，避免为库存、招聘、出租等业务各自设计一套接口。
- 请求和响应使用稳定字段，业务扩展字段放入 `attributes`。
- 所有用户可见错误使用中文友好提示，不暴露 SQL、堆栈、内部表字段、令牌或敏感数据。
- MVP 先覆盖注册登录、城市站、商家、统一供需信息发布与检索、认证、权益、消息、发布效果和后台审核。
...
- 分页方式：`page` + `pageSize`
- 用户与商家关系：当前产品为“一用户一商户”，小程序用户端接口原则上从 token 推导当前商户，不要求用户手动选择或切换 `merchantId`。历史接口中保留的 `merchantId` 参数只作为兼容字段或后台/运营明确指定商户范围使用。
...
| 404 | RESOURCE_NOT_FOUND | 供需信息不存在或已下架 |
...
- `managedMerchants` 为兼容历史命名保留数组结构，但当前业务最多返回一个 active 商户。
- 小程序端应把该商户视为当前用户唯一商户身份，不展示商户切换入口。
...
需求方向供需信息不再保留独立提交流程、独立列表或后台专用处理接口，统一走 `resources` 模型。
...
`GET /me/resources?direction=demand`
...
- 供需信息不是当前商家的：`该置顶券不能用于此供需信息`
- 供需信息未发布：`供需信息审核通过后才能置顶`
- 券已过期：`置顶券已过期`
