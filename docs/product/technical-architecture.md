...
- 没有实际接入 Redis；登录态使用自包含 HMAC Token，限流和任务调度主要在单进程内完成。
...
| 任务 | 默认周期 | 作用 | 幂等/风险 |
|---|---|---|---|
| 资源生命周期 | 1 小时 | 过期资源、到期提醒 | 消息有唯一约束；多实例仍会重复扫描 |
| 内容审核重试 | 1 分钟 | 处理失败或超时审核 | 数据库认领和状态控制；超限转人工 |
| 支付补偿 | 1 分钟 | 查询 pending 支付、补到账或关单 | 到账事务幂等；需控制微信查询量 |
| 地图行为清理 | 24 小时 | 删除超过保留期的明细 | 删除条件幂等；多实例会重复执行 |

这些 Scheduler 都在 API 进程内启动，没有分布式锁。单实例部署可直接使用；扩展到多实例前，应选择以下方案之一：只让一个实例启用任务、引入数据库抢占锁，或拆成独立 Worker。不能在未评估的情况下直接水平扩容所有实例。
...
`deploy/scripts/deploy-server.sh` 负责上传、安装、数据库迁移、systemd 重启和健康检查。首次部署发现生产配置尚未填写时会停止，要求运维补齐后再次执行。
...
自动任务和地图行为限流都是单进程状态，扩容后无法提供全局一致性。多实例部署前必须补任务抢占和共享限流。
...
2. 保证 Task 可重复执行并有数据库幂等约束。
3. 日志输出扫描、成功、失败和跳过数量。
4. 增加 `RunOnce` 单元测试。
5. 评估多实例重复执行、第三方配额和任务耗时是否超过周期。
