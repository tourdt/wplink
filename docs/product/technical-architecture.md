...
技术架构必须服务于一个核心目标：快速验证服装产业供需信息撮合闭环，并支持后续从织里扩展到广州、虎门、杭州等产业带。

设计原则：

- 前端先做微信小程序，不同时铺 App、H5 和 PC。
- 后端采用 go-zero 单体 API 服务起步，按领域模块清晰分包，暂不拆微服务。
- 供需信息、商家、需求、认证、权益、消息和数据统计复用公共能力。
- 新城市、新供需类型优先通过配置扩展，不新增独立系统。
...
首期只要求小程序端 + go-zero API + PostgreSQL 跑通。运营后台可以先用简单管理接口和低成本页面承接，等供需信息量和审核量上来后再完善独立后台体验。
...
| 城市站 | `city` | `city` | 城市站列表、供需类型启停、城市配置 |
| 商家 | `merchant` | `merchant` | 商家主页、资料、管理员绑定 |
| 供需信息 | `resource` | `resource` | 供应和需求方向供需信息的发布、审核、搜索、详情、刷新、成交、归档 |
| 认证与信用 | `verification` | `verification` | 商家认证、供需信息核验、信用标签 |
...
ER 设计选择 PostgreSQL，原因是统一供需信息模型大量依赖扩展字段：
...
- 涉及 `jsonb` 查询、供需信息搜索、统计汇总的 model 方法手写 SQL。
...
- 供需类型字段由后端配置驱动，发布页按 `fieldSchema` 渲染。
- 列表页统一使用供需信息卡片，招聘、出租、库存、工厂等只改变标签和字段展示。
...
| 页面 | 目标 |
|---|---|
| 首页 | 城市站入口、搜索、供需信息分类、推荐供需信息 |
| 搜索页 | 按关键词、供需类型、品类、认证状态筛选 |
| 供需信息详情 | 展示供需信息、商家、联系入口、收藏、举报 |
| 发布页 | 按供需类型配置动态发布供应或需求信息 |
| 商家主页 | 沉淀商家资料、认证、供需信息和信用标签 |
| 消息中心 | 审核、过期、认证和效果消息 |
...
- 搜索结果只展示 `published` 且未过期供需信息。
- 无结果时引导发布需求方向供需信息，仍使用统一供需信息接口。
...
- 认证商户可获得免费置顶券。
- 置顶券是商业化权益，不直接写死为供需信息字段。
- 后台可追踪每张券的来源、使用供需信息和有效期。
...
- 微信登录；手机号绑定后端接口预留，首发不开放。
- 城市站和供需类型配置。
- 商家主页和商家管理员绑定。
- 统一供需信息发布、审核、搜索、详情、联系、过期、归档。
- 搜索无结果空态；需求方向供需信息统一通过发布页和供需信息接口承接。
- 商家认证和轻量信用标签。
- 发布额度、刷新次数、置顶券。
- 消息中心基础通知。
- 发布效果数据统计。
...
5. 搭建 Vue 管理后台工程骨架。
6. 先跑通登录、城市站、供需信息列表、供需信息发布、供需信息详情和联系事件。
...
- 使用 PostgreSQL 支撑统一供需信息模型和 JSON 扩展字段。
- 采用单体 API 服务起步，不拆微服务。
- 按 DDD 领域上下文组织 API group 和 Logic 包。
- 供需类型配置驱动发布页和供需信息展示，避免为招聘、出租、库存等业务复制系统。
