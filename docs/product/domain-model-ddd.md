...
- 供需信息是平台核心抽象，货源、库存、工厂、订单、招聘、出租、服务都是供需类型。
- 新业务优先通过供需类型配置扩展，而不是新增独立系统。
...
| 术语 | 定义 |
|---|---|
| 用户 | 使用平台的自然人账号，可以浏览、搜索、联系、发布需求方向供需信息或管理商家。 |
| 商家 | 工厂、档口、库存商、服务商、采购商等经营主体。 |
| 商家管理员 | 代表商家维护主页、发布供需信息、查看效果数据的用户。 |
| 供需信息 | 可被搜索、展示和联系的供应或需求，是平台核心业务对象。 |
| 供需类型 | 供需信息的业务分类，如库存、货源、工厂产能、订单、招聘、出租、服务。 |
| 供需类型配置 | 定义某类供需信息的字段、有效期、审核规则、展示模板、排序权重等。 |
| 需求方向供需信息 | 买家、主播、电商卖家、订单方发布的找货、找厂或找服务供需信息，属于 `Resource` 聚合。 |
| 城市站 | 织里、广州、虎门、杭州等产业带运营单元。 |
| 认证 | 平台对商家、库存、服务能力等真实性进行核验。 |
| 信用记录 | 认证、成交反馈、投诉、平台核实等事实形成的信用依据。 |
| 联系行为 | 电话点击、微信复制、进入商家主页、分享等用户联系动作。 |
| 发布效果 | 供需信息发布后的曝光、浏览、联系、收藏、分享、成交反馈等数据。 |
| 权益 | 商家因认证、会员、运营赠送或购买获得的发布额度、刷新次数、置顶券等。 |
| 置顶券 | 可让指定供需信息获得一定时长置顶展示的权益凭证。 |

### 2.1 账号与商家关系约束

当前产品阶段采用“一用户一商户”模型：一个普通用户账号只对应一个有效商户主体，一个商户主体也只由该账号作为 owner 管理。小程序端不提供商户切换、多商户托管或手动选择商户身份能力。

落地要求：

- 登录或首次进入商家工作台时，系统自动创建或恢复该用户唯一的默认商户。
- 面向小程序用户的商家工作台、发布、权益、认证、我的发布等能力，原则上应从登录 token 推导当前商户，不把 `merchantId` 作为用户可选择的业务范围。
- `merchant_admin_bindings` 可以作为权限关系和后续扩展的技术表存在，但当前业务必须保证每个普通用户最多只有一个 active 商户绑定。
- 运营后台可以按商户 ID 查询和管理数据，但这不代表小程序端支持一个用户管理多个商户。
...
- 管理供需信息、供需类型配置和供需信息生命周期。
- 支持不同供需类型复用发布、审核、搜索、联系、过期、归档能力。
- 保证供需信息状态流转合法。
...
- 统计供需信息曝光、详情浏览、电话点击、微信复制、收藏、分享、成交反馈。
- 向商家和运营展示发布效果。
...
- 供需信息上下文是业务核心，其他上下文围绕供需信息产生认证、搜索、联系、撮合、效果数据和权益消耗。
- 商家上下文提供供需信息归属和信任承载。
- 城市站上下文提供本地化配置，不复制业务系统。
- 认证与信用上下文通过事实记录影响供需信息展示和商家信任。
...
- 供需信息必须归属一个商家。
- 供需信息必须有供需类型。
- 供需信息字段必须满足供需类型配置的必填规则。
- 待审核供需信息不能进入前台搜索和列表。
- 已过期、已下架、已归档供需信息默认不进入搜索和列表。
- 关键字段变更后必须重新审核。
- 置顶不能改变审核状态，也不能让违规供需信息展示。
...
- 定义某类供需信息的字段、展示、审核、有效期和运营规则。
- 支持新业务通过配置扩展。

核心不变量：

- 类型编码全局唯一。
- 启用中的供需类型必须有字段模板和默认有效期。
- 必填字段必须存在于字段模板中。
- 供需类型停用后，不影响历史供需信息归档和查看。
...
| 值对象 | 字段 | 说明 |
|---|---|---|
| ContactInfo | name, phone, wechat | 供需信息或商家联系方式。 |
| CityCode | code, name | 城市站编码和名称。 |
| ResourceValidity | publishedAt, expiresAt, archivedAt | 供需信息有效期。 |
...
### 10.2 买家搜索无结果后发布需求方向供需信息

1. 买家搜索供需信息。
2. 搜索结果为空或不足。
3. 搜索上下文记录 NoResultSearch 和供给缺口。
4. 用户通过发布页选择需求方向供需类型。
5. 供需信息上下文创建 `direction = demand` 的 Resource。
6. 后台供需信息审核通过后进入统一供需信息列表。
7. 用户联系供需信息或商家后进入发布效果统计。

### 10.3 认证商家使用置顶券

1. 商家选择已发布供需信息。
2. 权益上下文检查是否有可用置顶券。
3. 供需信息上下文检查供需信息是否已发布且未过期。
4. 权益上下文核销置顶券。
5. 供需信息获得置顶展示权重。
6. 操作日志记录核销行为。
...
- 信用只做事实标签，不做信用分。
- 消息只做审核、过期、认证和效果基础提醒。
- 需求方向供需信息并入 Resource 聚合，不再单独建模独立需求或后台对接聚合。
- 权益只做发布额度、刷新次数、置顶券。
...
3. 商家是否允许同时属于多个城市站。
4. 需求方向供需信息是否需要独立频道展示，还是仅作为供需信息列表筛选项。
5. 普通用户是否允许直接发布供需信息，还是必须先创建商家。
6. 运营代发供需信息是否在前台展示“平台代发”标记。
