...
当前产品阶段采用“一用户一商户”业务模型。`merchant_admin_bindings` 仍保留为用户和商户的权限关系表，但普通用户侧必须满足：

- 一个 active 用户最多只有一个 active 商户绑定。
- 一个普通商户默认只有一个 owner 用户管理。
- 小程序端不暴露多商户选择、切换或托管能力。
- 后台和数据修复脚本如需直接操作 `merchant_admin_bindings`，必须避免给同一普通用户写入多个 active 商户绑定；如发生历史脏数据，登录态恢复和 `/me` 响应应只选择一个 active 商户，并优先修正数据。
