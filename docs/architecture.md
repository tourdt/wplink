...
第一层是任务级 PostgreSQL 会话 Advisory Lock。Coordinator 从连接池取得专用 `*sql.Conn`，在同一数据库会话上执行 `pg_try_advisory_lock`、业务 Runner 和 `pg_advisory_unlock`。同类任务在整个数据库集群内同一时刻最多有一个执行者；不同任务锁号不同，仍可并发。进程退出或数据库连接断开后，PostgreSQL 会释放会话锁，其他 API 实例在下一次触发时自动接管。
...
协调日志字段使用代码中的 snake_case：`event`、`task`、`lock_key`、`instance_id`、`duration_ms`，错误场景附带脱敏后的 `error`。实例标识由主机名和 PID 组成，格式为 `<hostname>:<pid>`，只用于日志与审核租约诊断，不替代 PostgreSQL 会话锁。
...
进程停止时取消 Scheduler 根 Context，并调用四个 Scheduler 的 `Wait()`，等待已开始的本轮自然返回或超时；在此之前数据库连接池保持可用。专用连接正常关闭或异常断开都会使 PostgreSQL 最终释放会话 Advisory Lock。
